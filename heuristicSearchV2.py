...
def heuristicSearch(filename: str) -> None:
    ...
    nodeList = []                                        #create a nodeList to store unvisited nodes
    nodeList.append(startpoint)    #add the starting node to the nodeList
    ...
    while nodeList:
        ...
        nodeList.extend(newNodes)
        for node in newNodes:
            node.parent = currentNode
            node.calcHeuristic(endpoint)

        #remove the current node from the nodeList
